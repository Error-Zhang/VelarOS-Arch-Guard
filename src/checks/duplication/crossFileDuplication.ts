...
import { shortHash, stripCodeComments } from '../../utils/text'
import { collectSourceFilesForCheck } from '../_helpers'
...
const crossFileDuplication = defineCheck({
  ...
  run({ context, report }) {
    const windowSize =
      typeof context.options.windowSize === 'number' && context.options.windowSize > 1
        ? (context.options.windowSize)
        : DefaultWindowSize
    const minLineLength =
      typeof context.options.minLineLength === 'number' && context.options.minLineLength >= 0
    ...
    const files = collectSourceFilesForCheck(context, {
      roots: context.options.roots as readonly string[] | undefined,
      extensions: context.options.extensions as readonly string[] | undefined,
      include: context.options.include as readonly string[] | undefined,
      exclude: context.options.exclude as readonly string[] | undefined,
    }).filter((file) => !excludeFilePatterns.some((pattern) => pattern.test(file)))

    const windowOccurrences = new Map<string, Array<{ file: string; line: number }>>()

    for (const file of files) {
      const source = stripCodeComments(context.cache.readSource(file))
      const lines = source.split('\n')
      const significant: Array<{ text: string; line: number }> = []
      for (let i = 0; i < lines.length; i += 1) {
        const text = lines[i] ?? ''
        const trimmed = text.trim()
        if (trimmed.length < minLineLength) continue
        if (DefaultIgnorePatterns.some((pattern) => pattern.test(text))) continue
        significant.push({ text: trimmed, line: i + 1 })
      }
      if (significant.length < windowSize) continue
...
function parseRegexList(value: unknown): RegExp[] {
  if (!Array.isArray(value)) return []
  const result: RegExp[] = []
  for (const item of value) {
    if (item instanceof RegExp) result.push(item)
    else if (typeof item === 'string') result.push(new RegExp(item))
  }
  return result
}
