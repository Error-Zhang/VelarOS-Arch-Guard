...
const preferBooleanDataAttributes = defineCheck({
  ...
  description:
    'JSX data-* 属性直接传 boolean，由 React DOM 输出 true/false 字符串，避免手写字符串布尔三元。',
  verifies: [
...
interface BooleanDataAttributeReplacement {
  attributeName: string
  node: ts.Expression
  replacement: string
}

function readBooleanDataAttributeReplacement(
  attribute: ts.JsxAttribute,
  sourceFile: ts.SourceFile
): BooleanDataAttributeReplacement | undefined {
  ...
  if (ts.isCallExpression(rootExpression)) return readOptionalWhenDataAttributeReplacement(attributeName, rootExpression, sourceFile)
  if (!ts.isConditionalExpression(rootExpression)) return undefined
  ...
  if (whenTrue === 'true' && whenFalse === 'false') return {
      attributeName,
      node: conditional,
      replacement: formatBooleanCondition(conditional.condition, sourceFile),
    }
  if (whenTrue === 'false' && whenFalse === 'true') return {
      attributeName,
      node: conditional,
...
function readOptionalWhenDataAttributeReplacement(
  attributeName: string,
  ...
  if (callee.text === 'optionalWhen') {
    ...
    if (readBooleanAttributeLiteral(unwrapParensExpression(value)) !== 'true') return undefined
    return {
      attributeName,
      node: call,
  ...
  if (callee.text === 'optionalWhenLazy') {
    ...
    if (lazyValue !== 'true') return undefined
    return {
      attributeName,
      node: call,
      replacement: formatBooleanCondition(condition, sourceFile),
...
function readLazyBooleanAttributeLiteral(node: ts.Expression): 'true' | 'false' | undefined {
...
function readBooleanAttributeLiteral(node: ts.Expression): 'true' | 'false' | undefined {
  if (ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node)) {
    if (node.text === 'true' || node.text === 'false') return node.text
    return undefined
  }
