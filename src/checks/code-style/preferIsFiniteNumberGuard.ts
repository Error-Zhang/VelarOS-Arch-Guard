...
const preferIsFiniteNumberGuard = defineCheck({
  ...
  description:
    '将 `isNumber(x) && Number.isFinite(x)` 或手写 `typeof` + `Number.isFinite` 合并为 `isFiniteNumber(x)`；将 De Morgan `||` 形式合并为 `!isFiniteNumber(x)`。细则见 `finiteNumberGuardChains.ts`。',
  verifies: [
...
function planFiniteAndReplacement(parts: ts.Expression[], sourceFile: ts.SourceFile): FiniteAndPlan | undefined {
...
interface FiniteOrPlan {
  prefixTexts: string[]
  suffixTexts: string[]
  negFiniteCall: string
}

function planFiniteOrReplacement(parts: ts.Expression[], sourceFile: ts.SourceFile): FiniteOrPlan | undefined {
  for (let start = 0; start <= parts.length - 2; start += 1) {
    const slice = parts.slice(start, start + 2)
...
function formatFiniteOperand(expression: ts.Expression, sourceFile: ts.SourceFile): string {
