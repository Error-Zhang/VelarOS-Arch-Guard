...
const preferTableBranching = defineCheck({
  ...
  description:
    '（建议）同一 discriminant 的多个显式 if 分支：2~4 用 switch、≥5 用 table lookup。单个 guard/default 不提示。',
  verifies: [
  ...
  run({ context, report }) {
    ...
    const emit = (relativePath: string, sourceFile: ts.SourceFile, chain: BranchingChain): void => {
    ...
    for (const info of collectCodeStyleFiles(context)) {
      ...
      walk(sourceFile, (node) => {
        if (ts.isIfStatement(node) && !isElseIfStatement(node)) {
          ...
          if (chain) emit(info.relativePath, sourceFile, chain)
          return
        }
        if (
          ts.isBlock(node) ||
...
const GuidanceByKind: Record<BranchingChain['kind'], string> = {
  switch: 'Use switch for repeated explicit outcomes.',
  table: 'Use a table-driven lookup for five or more outcomes.',
}

function isElseIfStatement(node: ts.IfStatement): boolean {
...
function isLiteralCase(node: ts.Expression): boolean {
...
function readEqualityComparison(
  sourceFile: ts.SourceFile,
  expression: ts.Expression
): EqualityComparison | undefined {
  ...
  if (kind !== ts.SyntaxKind.EqualsEqualsEqualsToken) return undefined
  if (isLiteralCase(expr.left) && !isLiteralCase(expr.right)) return { caseText: expr.left.getText(sourceFile), discriminantText: expr.right.getText(sourceFile) }
  if (isLiteralCase(expr.right) && !isLiteralCase(expr.left)) return { caseText: expr.right.getText(sourceFile), discriminantText: expr.left.getText(sourceFile) }
  return undefined
}

function isSimpleExit(statement: ts.Statement | undefined): boolean {
  if (!statement) return false
  if (ts.isReturnStatement(statement) || ts.isThrowStatement(statement)) return true
  if (ts.isBlock(statement) && statement.statements.length === 1) return isSimpleExit(statement.statements[0])
  return false
}

function classifyChain(branchCount: number, hasDefault: boolean): BranchingChain['kind'] | undefined {
  if (branchCount >= TableBranchCount) return 'table'
  if (branchCount >= SwitchBranchCount || !hasDefault) return 'switch'
  return undefined
}

function collectElseIfChain(sourceFile: ts.SourceFile, ifStatement: ts.IfStatement): BranchingChain | undefined {
  const comparisons: EqualityComparison[] = []
  let discriminantText: string | undefined
  let current: ts.IfStatement | undefined = ifStatement
  let hasDefault = false

  while (current) {
    const comparison = readEqualityComparison(sourceFile, current.expression)
    if (!comparison) return undefined
    if (discriminantText && discriminantText !== comparison.discriminantText) return undefined
    if (!isSimpleExit(current.thenStatement)) return undefined
    discriminantText = comparison.discriminantText
    comparisons.push(comparison)

    const nextStatement: ts.Statement | undefined = current.elseStatement
    if (!nextStatement) break
    if (ts.isIfStatement(nextStatement)) {
      current = nextStatement
      continue
    }
    if (!isSimpleExit(nextStatement)) return undefined
    hasDefault = true
    break
  }

  if (comparisons.length < MinExplicitComparisonCount) return undefined

  const branchCount = comparisons.length + (hasDefault ? 1 : 0)
  const kind = classifyChain(branchCount, hasDefault)
  if (!kind || !discriminantText) return undefined
...
function collectSequentialChains(
  sourceFile: ts.SourceFile,
  statements: ts.NodeArray<ts.Statement>
): BranchingChain[] {
  const chains: BranchingChain[] = []
  for (let i = 0; i < statements.length; i += 1) {
    ...
    if (
      ...
      if (prevCmp?.discriminantText && prevCmp.discriminantText === currCmp?.discriminantText) continue
    }
...
function collectSequentialChainAt(
  sourceFile: ts.SourceFile,
  statements: ts.NodeArray<ts.Statement>,
  startIndex: number
): BranchingChain | undefined {
  const first = statements[startIndex]
  if (!first || !ts.isIfStatement(first) || first.elseStatement) return undefined
  const firstCmp = readEqualityComparison(sourceFile, first.expression)
  if (!firstCmp || !isSimpleExit(first.thenStatement)) return undefined

  const comparisons: EqualityComparison[] = []
  let i = startIndex
  while (i < statements.length) {
    const stmt = statements[i]
    if (!stmt || !ts.isIfStatement(stmt) || stmt.elseStatement) break
    const cmp = readEqualityComparison(sourceFile, stmt.expression)
    if (!cmp || cmp.discriminantText !== firstCmp.discriminantText) break
    if (!isSimpleExit(stmt.thenStatement)) break
