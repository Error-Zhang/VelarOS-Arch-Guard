    ...
    for (const info of collectCodeStyleFiles(context)) {
      ...
      walk(sourceFile, (node) => {
        if (ts.isConditionalExpression(node)) {
          const plan = readBooleanLiteralPair(
            node.whenTrue,
            node.whenFalse,
            node.condition,
            sourceFile
...
function asBooleanLiteralKind(node: ts.Node): 'true' | 'false' | undefined {
  if (node.kind === ts.SyntaxKind.TrueKeyword) return 'true'
  if (node.kind === ts.SyntaxKind.FalseKeyword) return 'false'
  return undefined
}

function readBooleanLiteralPair(
  a: ts.Node,
  b: ts.Node,
...
function singleReturnExpression(stmt: ts.Statement): ts.Expression | undefined {
...
interface IfBooleanReturnPlan {
  replacement: string
  start: number
  end: number
}

function readIfReturnsBooleanPair(
  node: ts.IfStatement,
  sourceFile: ts.SourceFile
): IfBooleanReturnPlan | undefined {
  ...
  if (!thenKind) return undefined

  let elseExpression: ts.Expression | undefined
  let endNode: ts.Node
  if (node.elseStatement) {
    ...
    if (isPartOfBooleanGuardChain(statements, idx)) return undefined
  ...
  if (!elseKind || elseKind === thenKind) return undefined
  return {
...
function formatBooleanCondition(
  condition: ts.Expression,
  positivePolarity: boolean,
  sourceFile: ts.SourceFile
): string {
  const unwrapped = unwrapParensExpression(condition)
  if (!positivePolarity) return formatNegatedExpression(unwrapped, sourceFile)
  if (isAlreadyBooleanExpression(unwrapped)) return unwrapped.getText(sourceFile)
  return `!!${formatUnaryOperand(unwrapped, sourceFile)}`
}

function formatNegatedExpression(expression: ts.Expression, sourceFile: ts.SourceFile): string {
...
function formatUnaryOperand(expression: ts.Expression, sourceFile: ts.SourceFile): string {
  const text = expression.getText(sourceFile)
  if (isUnarySafeExpression(expression)) return text
  return `(${text})`
}

function isAlreadyBooleanExpression(expression: ts.Expression): boolean {
  if (
...
function isUnarySafeExpression(expression: ts.Expression): boolean {
  return (
 ...
 */
function isPartOfBooleanGuardChain(statements: readonly ts.Statement[], index: number): boolean {
  const sibling = (offset: number): ts.Statement | undefined => statements[index + offset]
  const isBooleanIf = (stmt: ts.Statement | undefined): boolean => {
