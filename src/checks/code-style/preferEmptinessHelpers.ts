...
const ArrayReturningNamePattern =
  /(?:Artifacts|Branches|Candidates|Categories|Changes|Commands|Entries|Features|Files|Hints|Ids|IDs|Items|Keys|Labels|Lines|List|Lists|Manifests|Messages|Models|Nodes|Notes|Outputs|Paths|Ports|Results|Roles|Roots|Rows|Sessions|Snippets|Steps|Symbols|Tasks|Tools|Values|Windows)$/
        ...
        section.add({
          ...
          ...(violation.fix
            ? {
                ...
                applyFix: fixReplaceLengthCheck(
                  info.relativePath,
                  sourceFile,
...
interface RuntimeHints {
  arrayLikeNames: Set<string>
  stringLikeNames: Set<string>
  arrayReturningFunctionNames: Set<string>
}

interface DetectedViolation {
  ruleId: string
  text: string
  fix?: { replaceNode: ts.Node; replacement: string }
}
...
function detectViolation(
  ...
  if (pattern.lengthAccess.getText(sourceFile).includes('?.')) return undefined
...
function readEmptyStringComparison(node: ts.Node): ts.Expression | undefined {
  if (!ts.isBinaryExpression(node)) return undefined
  if (!isEqualityOperator(node.operatorToken.kind)) return undefined
  if (isEmptyStringLiteral(node.left)) return node.right
  if (isEmptyStringLiteral(node.right)) return node.left
  return undefined
}
...
function isLengthOperator(kind: ts.SyntaxKind): boolean {
  return (
...
function isOneLiteral(node: ts.Node): boolean {
  return ts.isNumericLiteral(node) && Number(node.text) === 1
}

interface LengthIsEmptyPattern {
  lengthAccess: ts.PropertyAccessExpression
  replaceNode: ts.Node
  /** `true` → 非空（`!isEmpty(x)`）；`false` → 空（`isEmpty(x)`） */
  preferNonEmpty: boolean
}

function readLengthIsEmptyPattern(node: ts.Node): LengthIsEmptyPattern | undefined {
  if (ts.isBinaryExpression(node) && isLengthOperator(node.operatorToken.kind)) {
    ...
    if (lenL && zR) {
      if (op === ts.SyntaxKind.EqualsEqualsToken || op === ts.SyntaxKind.EqualsEqualsEqualsToken) return { lengthAccess: L, replaceNode: node, preferNonEmpty: false }
      if (op === ts.SyntaxKind.ExclamationEqualsToken || op === ts.SyntaxKind.ExclamationEqualsEqualsToken) return { lengthAccess: L, replaceNode: node, preferNonEmpty: true }
      if (op === ts.SyntaxKind.GreaterThanToken) return { lengthAccess: L, replaceNode: node, preferNonEmpty: true }
      if (op === ts.SyntaxKind.LessThanEqualsToken) return { lengthAccess: L, replaceNode: node, preferNonEmpty: false }
      return undefined
    }
    if (lenL && oR) {
      if (op === ts.SyntaxKind.GreaterThanEqualsToken) return { lengthAccess: L, replaceNode: node, preferNonEmpty: true }
      if (op === ts.SyntaxKind.LessThanToken) return { lengthAccess: L, replaceNode: node, preferNonEmpty: false }
      return undefined
    }
    if (zL && lenR) {
      if (op === ts.SyntaxKind.EqualsEqualsToken || op === ts.SyntaxKind.EqualsEqualsEqualsToken) return { lengthAccess: R, replaceNode: node, preferNonEmpty: false }
      if (op === ts.SyntaxKind.ExclamationEqualsToken || op === ts.SyntaxKind.ExclamationEqualsEqualsToken) return { lengthAccess: R, replaceNode: node, preferNonEmpty: true }
      if (op === ts.SyntaxKind.LessThanToken) return { lengthAccess: R, replaceNode: node, preferNonEmpty: true }
      if (op === ts.SyntaxKind.GreaterThanEqualsToken) return { lengthAccess: R, replaceNode: node, preferNonEmpty: false }
      return undefined
    }
    if (oL && lenR && op === ts.SyntaxKind.LessThanEqualsToken) return { lengthAccess: R, replaceNode: node, preferNonEmpty: true }
    return undefined
  }
  if (
  ...
  if (isLengthAccess(node) && isBooleanContextNode(node)) return { lengthAccess: node, replaceNode: node, preferNonEmpty: true }
  return undefined
}

function isLengthAccess(node: ts.Node): node is ts.PropertyAccessExpression {
  return ts.isPropertyAccessExpression(node) && node.name.text === 'length'
}

function isTrimCall(node: ts.Node): boolean {
  return (
...
function isBooleanContextNode(node: ts.Node): boolean {
  ...
  if (ts.isForStatement(parent) && parent.condition === expression) return true
  if (
...
function helperFor(expression: ts.Expression, hints: RuntimeHints): 'isEmpty' | 'isBlank' | undefined {
  if (isTrimCall(expression)) return 'isBlank'
  if (ts.isIdentifier(expression)) {
    if (hints.stringLikeNames.has(expression.text)) return 'isEmpty'
    if (hints.arrayLikeNames.has(expression.text)) return 'isEmpty'
  }
  if (looksLikeArrayFactory(expression, hints)) return 'isEmpty'
  if (looksLikeStringFactory(expression)) return 'isEmpty'
  if (ts.isPropertyAccessExpression(expression) || ts.isElementAccessExpression(expression)) return 'isEmpty'
  return undefined
}

function looksLikeArrayFactory(expression: ts.Expression, hints: RuntimeHints): boolean {
  if (ts.isArrayLiteralExpression(expression)) return true
  if (!ts.isCallExpression(expression)) return false
  const callee = expression.expression
  if (ts.isIdentifier(callee)) return hints.arrayReturningFunctionNames.has(callee.text)
  if (!ts.isPropertyAccessExpression(callee)) return false
  if (ArrayFactoryMethodNames.has(callee.name.text)) return true
  if (ts.isIdentifier(callee.expression)) {
    if (callee.expression.text === 'Array' && callee.name.text === 'from') return true
    if (callee.expression.text === 'Object' && ['entries', 'keys', 'values'].includes(callee.name.text)) return true
  }
  const name = readCalleeName(callee)
  return Boolean(name && ArrayReturningNamePattern.test(name))
}

function looksLikeStringFactory(expression: ts.Expression): boolean {
...
function readCalleeName(callee: ts.Expression): string | undefined {
  if (ts.isIdentifier(callee)) return callee.text
  if (ts.isPropertyAccessExpression(callee)) return callee.name.text
  return undefined
}

function isArrayTypeNode(node: ts.TypeNode | undefined): boolean {
...
function isStringTypeNode(node: ts.TypeNode | undefined): boolean {
  return node?.kind === ts.SyntaxKind.StringKeyword
}

function collectRuntimeHints(sourceFile: ts.SourceFile): RuntimeHints {
  ...
  const collectNames = (node: ts.Node): void => {
    ...
    if (ts.isParameter(node) && ts.isIdentifier(node.name)) {
      if (isArrayTypeNode(node.type)) hints.arrayLikeNames.add(node.name.text)
      if (isStringTypeNode(node.type)) hints.stringLikeNames.add(node.name.text)
    }
...
function fixReplaceLengthCheck(
  relativePath: string,
  sourceFile: ts.SourceFile,
  node: ts.Node,
  replacement: string,
