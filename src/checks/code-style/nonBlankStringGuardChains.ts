...
import { enclosingConditionalIfDirectCondition, unwrapParensExpression as unwrapParens } from './_shared'
...
function tryParseTypeofEqualsStringOperand(node: ts.Node): ts.Expression | undefined {
  if (!ts.isBinaryExpression(node)) return undefined
  const op = node.operatorToken.kind
  if (op !== ts.SyntaxKind.EqualsEqualsEqualsToken && op !== ts.SyntaxKind.EqualsEqualsToken) return undefined
  if (ts.isTypeOfExpression(node.left) && ts.isStringLiteral(node.right) && node.right.text === 'string') return node.left.expression
  if (ts.isTypeOfExpression(node.right) && ts.isStringLiteral(node.left) && node.left.text === 'string') return node.right.expression
  return undefined
}

function readGlobalIsStringCallArg(expr: ts.Expression): ts.Expression | undefined {
  const u = unwrapParens(expr)
  if (!ts.isCallExpression(u) || u.arguments.length !== 1) return undefined
  if (!ts.isIdentifier(u.expression) || u.expression.text !== 'isString') return undefined
  const arg = u.arguments[0]
  return arg ? unwrapParens(arg) : undefined
}

function extractStringGuardSubject(part: ts.Expression): ts.Expression | undefined {
...
function stripLeadingDoubleBang(expr: ts.Expression): ts.Expression | undefined {
  const e = unwrapParens(expr)
  if (!ts.isPrefixUnaryExpression(e) || e.operator !== ts.SyntaxKind.ExclamationToken) return undefined
  const once = unwrapParens(e.operand)
  if (!ts.isPrefixUnaryExpression(once) || once.operator !== ts.SyntaxKind.ExclamationToken) return undefined
  return unwrapParens(once.operand)
}

function unwrapBooleanCall(expr: ts.Expression): ts.Expression | undefined {
  const e = unwrapParens(expr)
  if (!ts.isCallExpression(e) || e.arguments.length !== 1) return undefined
  const callee = unwrapParens(e.expression)
  if (!ts.isIdentifier(callee) || callee.text !== 'Boolean') return undefined
  const arg = e.arguments[0]
  return arg ? unwrapParens(arg) : undefined
}

function parseTrimCallSubject(expr: ts.Expression): ts.Expression | undefined {
  const e = unwrapParens(expr)
  ...
  if (recv.name.text !== 'trim') return undefined
  return recv.expression
}

/** `!!x.trim()`、`x.trim()`、`Boolean(x.trim())` → `x`（须为零参 `.trim()`）。 */
function readTrimTruthySubject(expr: ts.Expression): ts.Expression | undefined {
  const u = unwrapParens(expr)
...
function matchNonBlankStringPositiveAndPair(parts: ts.Expression[], sourceFile: ts.SourceFile): ts.Expression | undefined {
  ...
  for (const [guardPart, trimPart] of orders) {
    ...
    if (subjStr && subjTrim && expressionsTextEqual(subjStr, subjTrim, sourceFile)) return subjStr
  }
  return undefined
}

function isEmptyStringLiteralExpression(expr: ts.Expression): boolean {
  const u = unwrapParens(expr)
  return ts.isStringLiteral(u) && u.text === ''
}

/** `isString(x)` / `typeof x === 'string'` + `x.trim()` + `: ''` 三元（whenTrue 须为零参 `.trim()`）。 */
function matchTrimmedStringOrEmptyTernary(node: ts.ConditionalExpression, sourceFile: ts.SourceFile): ts.Expression | undefined {
  ...
  if (!trimRecv) return undefined
  if (!expressionsTextEqual(subjCond, trimRecv, sourceFile)) return undefined
  if (!isEmptyStringLiteralExpression(node.whenFalse)) return undefined
  return subjCond
}

/** forbid-raw：`typeof x === 'string'` 为 **`trimmedStringOrEmpty`** 三元条件时跳过。 */
function shouldSkipTypeofStringEqForTrimmedStringOrEmptyTernary(node: ts.BinaryExpression, sourceFile: ts.SourceFile): boolean {
  const op = node.operatorToken.kind
  if (op !== ts.SyntaxKind.EqualsEqualsEqualsToken && op !== ts.SyntaxKind.EqualsEqualsToken) return false
  if (!tryParseTypeofEqualsStringOperand(unwrapParens(node))) return false
...
function shouldSkipTypeofStringEqForNonBlankAndPair(node: ts.BinaryExpression, sourceFile: ts.SourceFile): boolean {
  const op = node.operatorToken.kind
  if (op !== ts.SyntaxKind.EqualsEqualsEqualsToken && op !== ts.SyntaxKind.EqualsEqualsToken) return false
  if (!tryParseTypeofEqualsStringOperand(unwrapParens(node))) return false
  ...
  for (let i = 0; i <= parts.length - 2; i += 1) {
    ...
    if (!matchNonBlankStringPositiveAndPair(slice, sourceFile)) continue
...
export {
  ...
  matchNonBlankStringPositiveAndPair,
  matchTrimmedStringOrEmptyTernary,
  shouldSkipTypeofStringEqForNonBlankAndPair,
  shouldSkipTypeofStringEqForTrimmedStringOrEmptyTernary,
}
export { unwrapParensExpression as unwrapParens } from './_shared'
