...
const forbidClassNameJoinArrays = defineCheck({
  ...
  description:
    'className 不得使用若干 string 字面量数组再 `.join(\' \')` 拼接；请改用 CSS Module、样式常量或上层布局组件。',
  verifies: [
...
function reportClassNameJoinIfNeeded(
  ...
  if (!ts.isIdentifier(node.name) || node.name.escapedText !== 'className') return
  if (!node.initializer || !ts.isJsxExpression(node.initializer)) return
  const expr = node.initializer.expression
  if (expr === undefined || !isJoinSpaceArray(expr)) return
...
function isJoinSpaceArray(expr: ts.Expression): boolean {
  if (!ts.isCallExpression(expr)) return false
  const callee = expr.expression
  if (!ts.isPropertyAccessExpression(callee)) return false
  if (callee.name.text !== 'join') return false
  ...
  if (!ts.isArrayLiteralExpression(target) || target.elements.length < 2) return false
  return true
}
