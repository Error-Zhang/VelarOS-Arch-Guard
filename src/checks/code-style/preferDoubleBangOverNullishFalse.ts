...
const preferDoubleBangOverNullishFalse = defineCheck({
  ...
  description:
    '将 `expr ?? false` 改为 `!!expr`（复杂左操作数自动加括号）；左侧已是 `!!…` 时删去多余的 `?? false`。',
  verifies: [
    '识别 `??` 且右操作数为字面量 `false`。',
  ...
  run({ context, report }) {
    ...
    for (const info of collectCodeStyleFiles(context)) {
      ...
      walk(sourceFile, (node) => {
        ...
        if (node.right.kind !== ts.SyntaxKind.FalseKeyword) return
        ...
        section.add({
          ...
          applyFix: fixReplaceNullishFalse(info.relativePath, sourceFile, node, replacement),
        })
      })
    }
  },
...
function fixReplaceNullishFalse(
  relativePath: string,
...
function formatDoubleBangReplacement(left: ts.Expression, sourceFile: ts.SourceFile): string {
  if (isAlreadyDoubleBooleanCoercion(left)) return left.getText(sourceFile)
...
function isAlreadyDoubleBooleanCoercion(expr: ts.Expression): boolean {
...
function shouldWrapLeftInParensForDoubleBang(expr: ts.Expression): boolean {
  const u = unwrapParens(expr)
  if (
    ts.isIdentifier(u) ||
  ...
  if (ts.isLiteralExpression(u)) return false
  return true
}
