...
import { readTypeofObjectStrictNotNullMerge } from './forbidRawRuntimeTypeGuards'
...
const forbidRedundantStrictLiteralComparison = defineCheck({
  ...
  description:
    '标记 ===/!== 与 undefined、null、true、false 字面量，以及 typeof x 与字符串 "undefined" 的比较（含左右互换）。',
  verifies: [
...
function isNullNeInTypeofObjectMerge(sourceFile: ts.SourceFile, node: ts.BinaryExpression): boolean {
  const parent = node.parent
  if (!ts.isBinaryExpression(parent) || parent.operatorToken.kind !== ts.SyntaxKind.AmpersandAmpersandToken) return false
  return readTypeofObjectStrictNotNullMerge(parent, sourceFile) !== undefined
}

function readStrictLiteralComparable(node: ts.Node): 'undefined' | 'null' | 'true' | 'false' | undefined {
...
function readTypeofComparedToUndefinedStringLiteral(node: ts.BinaryExpression): boolean {
  if (ts.isTypeOfExpression(node.left) && ts.isStringLiteral(node.right) && node.right.text === 'undefined') return true
  if (ts.isTypeOfExpression(node.right) && ts.isStringLiteral(node.left) && node.left.text === 'undefined') return true
  return false
}
