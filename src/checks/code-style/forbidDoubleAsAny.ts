...
const forbidDoubleAsAny = defineCheck({
  ...
  description:
    '历史 opt-in 规则：禁止 double-as-any 断言；VelarOS 默认 checks 不启用，动态边界可直接用 any。',
  ...
  run({ context, report }) {
    ...
    for (const info of collectCodeStyleFiles(context)) {
      ...
      walk(sourceFile, (node) => {
        ...
        if (node.expression.type.kind !== ts.SyntaxKind.AnyKeyword) return
        ...
        section.add({
          ...
          suggestion:
            'Keep double-as-any only at true dynamic boundaries; otherwise introduce a typed boundary helper.',
        })
      })
    }
  },
