...
import { SilentCatchSuppressionMarkers } from './requireErrorLogging'
...
const forbidSwallowedErrors = defineCheck({
  ...
  description:
    '空 `catch {}` 与空 `.catch(() => {})` 静默吞错计数棘轮；写明原因的 silent-catch-ok 注解视为合法。',
  verifies: [
    '识别 body 无语句的 `catch { }`。',
    ...
    for (const info of collectCodeStyleFiles(context)) {
      ...
      walk(sourceFile, (node) => {
        if (ts.isCatchClause(node)) {
          ...
          if (hasSuppressionMarker(sourceFile, node.parent)) return
        ...
        if (ts.isCallExpression(node) && isEmptyPromiseCatch(node)) {
          if (hasSuppressionMarker(sourceFile, enclosingStatement(node) ?? node)) return
          const line = lineOf(sourceFile, node)
          section.add({
...
function isEmptyPromiseCatch(call: ts.CallExpression): boolean {
...
function isEmptyHandler(node: ts.Expression): boolean {
  if (ts.isArrowFunction(node)) {
    const body = node.body
    if (ts.isBlock(body)) return body.statements.length === 0
    return isNoOpExpression(body)
  }
  if (ts.isFunctionExpression(node)) return node.body.statements.length === 0
  return false
}

/** `() => undefined` / `() => void 0` 视为等价空吞。 */
function isNoOpExpression(node: ts.Node): boolean {
...
function hasSuppressionMarker(sourceFile: ts.SourceFile, scope: ts.Node): boolean {
