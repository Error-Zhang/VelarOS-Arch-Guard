 ...
 */
function findUnknownOptions(args: ParsedArgs): UnknownOption[] {
  const flatten = (key: string): string => key.replaceAll(/[-_]/g, '').toLowerCase()
  const byFlattened = new Map<string, string>()
  for (const known of KnownOptions) byFlattened.set(flatten(known), known)
