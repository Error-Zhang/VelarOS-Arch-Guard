...
import { collectAllChecks, effectiveSeverity } from '../../core/runner'
import { type ParsedArgs, toBoolean, toString } from '../argv'
...
async function listCommand(args: ParsedArgs): Promise<number> {
  ...
  if (allChecks.length === 0) {
    if (toBoolean(args.options.json)) {
      ...
      console.info(
        'arch-guard: no checks registered. Add a plugin or `checks` array to arch-guard.config.'
      )
    }
    return 0
  }
  ...
  if (toBoolean(args.options['ids-only'])) {
    for (const check of allChecks) console.info(check.id)
    return 0
  }
  ...
  if (toBoolean(args.options['by-tag'])) {
    const byTag = new Map<string, typeof allChecks>()
