...
async function doctorCommand(args: ParsedArgs): Promise<number> {
  ...
  for (const plugin of config.plugins) {
    if (typeof plugin.validate === 'function') {
      try {
        ...
        issues.push(
          `plugin "${plugin.name}" validate failed: ${error instanceof Error ? error.message : String(error)}`
        )
      }
    }
  }
  ...
  for (const [id, count] of idCounts) {
    if (count > 1) issues.push(`Check id "${id}" appears ${count} times; ids must be unique.`)
  }
