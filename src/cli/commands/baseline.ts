...
import { buildCheckFilter, type CheckFilterInput, passesFilter } from '../../core/filter'
import {
  collectAllChecks,
  effectiveSeverity,
  isFilterActive,
...
import { violationKey } from '../../core/violation'
...
interface BaselineCommandContext {
  config: ResolvedConfig
  scopedConfig: ResolvedConfig
  baselinePath: string
  filter: CheckFilterInput
  fileScopeActive: boolean
  /**
   * 要求了文件作用域、但解析下来一个可扫文件都没有时的说明；否则 `undefined`。
   * 写命令据此拒绝，读命令据此加告示。
   */
  emptyScopeNotice: string | undefined
  scopeFiles: readonly string[]
  dryRun: boolean
  reporters: Reporter[]
}

async function loadBaselineCommandContext(args: ParsedArgs): Promise<BaselineCommandContext> {
  ...
  return {
    ...
    baselinePath:
      toString(args.options['baseline-path']) ?? resolveBaselinePath(config.rootDir),
    filter: {
 ...
 */
function refuseEmptyScope(ctx: BaselineCommandContext, action: string): boolean {
  if (ctx.emptyScopeNotice === undefined) return false
  console.info(
    `arch-guard: ${ctx.emptyScopeNotice}. \`baseline ${action}\` did nothing; ` +
 ...
 */
function isScopeNarrowed(ctx: BaselineCommandContext): boolean {
  if (ctx.fileScopeActive) return true
  return (ctx.filter.only?.length ?? 0) > 0 || (ctx.filter.tags?.length ?? 0) > 0
}

/** 把「这次到底冻了哪一片」讲成一句人话，取代 scoped / whole-repository 的二选一。 */
function describeUpdateScope(ctx: BaselineCommandContext, narrowed: boolean): string {
  ...
  if (skip.length > 0) parts.push(`minus --skip ${skip.join(',')}`)
 ...
 */
async function baselineUpdate(args: ParsedArgs): Promise<number> {
  const ctx = await loadBaselineCommandContext(args)
  if (refuseEmptyScope(ctx, 'update')) return 0
 ...
 */
async function baselinePrune(args: ParsedArgs): Promise<number> {
  const ctx = await loadBaselineCommandContext(args)
  if (refuseEmptyScope(ctx, 'prune')) return 0
  if (ctx.fileScopeActive || isFilterActive(ctx.filter)) {
    console.error(
      'arch-guard: `baseline prune` refuses to run under --file / --changed / --staged / --only / --skip / --tag: ' +
  ...
  const surrenderedQuota =
    totalCapacity(existing.filter((entry) => entry.contentDigest !== undefined)) -
  ...
  if (matched.length === 0 && !toBoolean(args.options.force)) {
    console.error(
      `arch-guard: refusing to prune all ${stale.length} entries — this run matched none of them, ` +
  ...
  if (surrenderedQuota > 0) {
    console.info(
      `arch-guard: ${surrenderedQuota} unused waiver slot(s) on still-matched entries will be given back ` +
  ...
  if (contentMismatches.length > 0) {
    console.info(
      `arch-guard: ${contentMismatches.length} of them are content mismatches ` +
...
function totalCapacity(entries: readonly BaselineEntry[]): number {
 ...
 */
async function baselineMigrate(args: ParsedArgs): Promise<number> {
  const ctx = await loadBaselineCommandContext(args)
  if (refuseEmptyScope(ctx, 'migrate')) return 0
  if (ctx.fileScopeActive || isFilterActive(ctx.filter)) {
  ...
  for (const entry of existing) {
    ...
    if (live.length > 1) collidingFingerprints += 1
    // 同指纹可能盖着多条现存违规；只要有一条与冻结内容一致，就是干净的匹配。
    const exact = live.find(
      (violation) => contentDigestForMessage(violation.message) === frozenDigest
    )
    if (exact) {
      migrated.push({ ...entry, contentDigest: frozenDigest })
  ...
  console.info(
    `arch-guard: baseline migrate — ${attached} entr${attached === 1 ? 'y' : 'ies'} gained a content digest ` +
  ...
  if (collidingFingerprints > 0) {
    console.info(
      `arch-guard: ${collidingFingerprints} fingerprint${collidingFingerprints === 1 ? '' : 's'} matched more than ` +
        'one live violation — that is the collision this digest exists to stop.'
    )
  }
  if (mismatches.length > 0) {
    console.info(
      `arch-guard: ${mismatches.length} entr${mismatches.length === 1 ? 'y' : 'ies'} freeze code that no longer exists ` +
...
function printMigrationImpact(
  ...
  console.info(
    `arch-guard: ${losing.length} violation${losing.length === 1 ? '' : 's'} waived today will STOP being waived ` +
 ...
 */
async function baselineCheck(args: ParsedArgs): Promise<number> {
  const ctx = await loadBaselineCommandContext(args)
  ...
  if (ctx.emptyScopeNotice !== undefined) {
    console.info(
      `arch-guard: ${ctx.emptyScopeNotice} — no file was scanned; only checks that ignore the ` +
  ...
  if (uncovered > 0) {
    console.error(
      `arch-guard: ${uncovered} violation${uncovered === 1 ? '' : 's'} not covered by ${ctx.baselinePath}. ` +
  ...
  if (ctx.emptyScopeNotice !== undefined) {
    console.info(
      `arch-guard: nothing was scanned, so there was nothing for ${ctx.baselinePath} to cover.`
    )
    return 0
  }
  ...
  if (status && status.staleCount > 0) {
    console.info(
      `arch-guard: baseline covers every violation. ${status.staleCount} entr${status.staleCount === 1 ? 'y is' : 'ies are'} ` +
...
function buildScopePredicate(
  ctx: BaselineCommandContext,
  observed: readonly BaselineEntry[]
): (entry: BaselineEntry) => boolean {
  const filter = buildCheckFilter(ctx.filter)
  const ranCheckIds = new Set(
    collectAllChecks(ctx.config)
  ...
  return (entry: BaselineEntry): boolean => {
    if (!ranCheckIds.has(entry.checkId)) return false
    if (!matchesFile) return true
    if (entry.file !== undefined) return matchesFile(normalizeEntryFile(entry.file))
 ...
 */
function normalizeEntryFile(file: string): string {
  return file.replaceAll('\\', '/').replace(/^\.\/+/, '').replace(/\/+$/, '')
}

/** `--file` 接受文件 / 目录 / glob 三种形态，这里统一成一个 predicate。 */
function buildFileScopeMatcher(scopeFiles: readonly string[]): (file: string) => boolean {
  const globs: string[] = []
  ...
  for (const raw of scopeFiles) {
    ...
    if (!entry) continue
    if (/[*?[\]{}]/.test(entry)) globs.push(entry)
    else if (/\.[cm]?[jt]sx?$/.test(entry)) exact.add(entry)
...
interface EntryDiff {
  ...
  keptCount: number
  /** 指纹两边都在，只是补上了内容摘要（首次迁移的常态，不是新债）。 */
  digestAddedCount: number
  /** 指纹两边都在，但内容变了——同一行换成了另一条违规，值得人看一眼。 */
  contentChanged: BaselineEntry[]
  /** 同一豁免键的配额变大：同形态违规被复制了更多份，多出来的份数也是新债。 */
  grown: Array<{ entry: BaselineEntry; before: number; after: number }>
}
 ...
 */
function diffEntries(before: readonly BaselineEntry[], after: readonly BaselineEntry[]): EntryDiff {
  const beforeCounts = capacityByWaiverKey(before)
  const beforeByFingerprint = new Map<string, BaselineEntry[]>()
  for (const entry of before) {
    const key = keyFromBaselineEntry(entry)
    const bucket = beforeByFingerprint.get(key)
    if (bucket) bucket.push(entry)
    else beforeByFingerprint.set(key, [entry])
  }
  const afterFingerprints = new Set(after.map(keyFromBaselineEntry))

  const added: BaselineEntry[] = []
  const contentChanged: BaselineEntry[] = []
  const grown: EntryDiff['grown'] = []
  ...
  for (const entry of after) {
    ...
    if (previous.every((old) => old.contentDigest === undefined)) digestAddedCount += 1
    else contentChanged.push(entry)
  }
  ...
  return {
    ...
    removed: before.filter(
      (entry) =>
        !afterFingerprints.has(keyFromBaselineEntry(entry)) &&
...
function capacityByWaiverKey(entries: readonly BaselineEntry[]): Map<string, number> {
...
interface UpdateSummaryInput {
  /** 写入面真的比全仓小（`--skip` 不算，见 {@link isScopeNarrowed}）。 */
  narrowed: boolean
  /** 人话版的作用域描述。 */
  scopeLabel: string
  diff: EntryDiff
  preservedCount: number
  /** 本次 run 报了、但落在作用域外因而**没有**被冻的违规数。 */
...
function printUpdateSummary(input: UpdateSummaryInput): void {
  const { diff } = input
  const grownDelta = diff.grown.reduce((sum, item) => sum + (item.after - item.before), 0)
  console.info(
    `arch-guard: baseline update (${input.scopeLabel}) — ` +
  ...
  if (input.skippedOutOfScope > 0) {
    console.info(
      `arch-guard: ${input.skippedOutOfScope} violation(s) reported by this run fall outside the scope and were ` +
  ...
  if (diff.contentChanged.length > 0) {
    console.info(
      'arch-guard: these entries kept their fingerprint but froze different code — the old entry was ' +
  ...
  if (diff.grown.length > 0) {
    console.info(
      `arch-guard: ${diff.grown.length} entr${diff.grown.length === 1 ? 'y' : 'ies'} now waive MORE occurrences of the ` +
  ...
  if (input.dryRun) console.info('arch-guard: --dry-run, baseline not written.')
}

const EntrySampleLimit = 20

function printEntrySample(entries: readonly BaselineEntry[]): void {
  const byCheck = new Map<string, number>()
  for (const entry of entries) byCheck.set(entry.checkId, (byCheck.get(entry.checkId) ?? 0) + 1)
