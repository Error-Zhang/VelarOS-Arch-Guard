...
function getScriptKindFromFile(filePath: string): ts.ScriptKind {
...
function parseSourceFile(filePath: string, source: string): ts.SourceFile {
  return ts.createSourceFile(
    ...
    true,
    getScriptKindFromFile(filePath)
  )
}
...
/** 是否为字符串叶子（含模板字面量无替换）。 */
function isStringLeafExpression(expression: ts.Expression): boolean {
...
function getColumnNumber(sourceFile: ts.SourceFile, position: number): number {
...
function forEachComment(
  ...
  source: string,
  visitor: (commentText: string, tokenPos: number, rawText: string) => void
): void {
...
function normalizeCommentText(rawComment: string): string {
  return rawComment
    .replace(/^\/\//, '')
    .replace(/^\/\*/, '')
    .replace(/\*\/$/, '')
...
function hasNamedExport(source: string, exportName: string): boolean {
  ...
  return (
    new RegExp(
      `export\\s+(?:function|const|let|var|class|enum|interface|type)\\s+${escapedName}\\b`
    ).test(source) ||
