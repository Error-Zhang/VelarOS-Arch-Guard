 ...
 * - 其它情况视为外部包导入。
 */
function createAliasResolver(options: AliasResolverOptions) {
  const sortedAliases = [...options.aliases].sort((a, b) => b.prefix.length - a.prefix.length)

  return function resolveImportSpecifier(fromFile: string, specifier: string): string | null {
    if (specifier.startsWith('.')) return resolve(dirname(fromFile), specifier)
    for (const alias of sortedAliases) {
      if (specifier === alias.prefix) return resolve(options.rootDir, alias.target)
      if (specifier.startsWith(`${alias.prefix}/`)) return resolve(options.rootDir, alias.target, specifier.slice(alias.prefix.length + 1))
    }
    return null
  }
}
