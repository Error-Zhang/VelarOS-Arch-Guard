...
import type { SharedCache } from './context'
...
function splitReasonPayload(rest: string): { scopesPart: string; reason: string } | null {
  const idxEnglish = rest.indexOf('Reason:')
  if (idxEnglish >= 0) return {
...
function reasonLenOk(reason: string, scopes: readonly string[]): boolean {
  const n = [...reason].length
  if (scopes.includes(ARCH_GUARD_SUSPEND_ALL)) return n >= WILDCARD_REASON_MIN
  return n >= LINE_SUSPEND_REASON_MIN
}

function parseSuspendBody(body: string, fileScope: boolean): ParsedSuspend | null {
  const split = splitReasonPayload(body.trim())
  if (!split || split.reason.length === 0) return null
  ...
  if (fileScope && scopes.includes(ARCH_GUARD_SUSPEND_ALL)) return null

  if (!reasonLenOk(split.reason, scopes)) return null

  return { scopes, reason: split.reason, fileScope }
}

function tryParseSuspendFromCommentSlice(comment: string, fileScope: boolean): ParsedSuspend | null {
...
function collectSuspendsFromSourceLine(rawLine: string): ParsedSuspend[] {
  const results: ParsedSuspend[] = []
...
function scopeMatchesViolation(scope: string, violation: Violation): boolean {
  if (scope === ARCH_GUARD_SUSPEND_ALL) return true
...
function suspendCoversViolation(parsed: ParsedSuspend, violation: Violation): boolean {
  return parsed.scopes.some((s) => scopeMatchesViolation(s, violation))
}

/** 行 L 为 1-based。 */
function parseLineScopesSuspends(lines: readonly string[], lineOneBased: number): ParsedSuspend[] {
  const out: ParsedSuspend[] = []
...
function parseFileHeadFileSuspends(lines: readonly string[]): ParsedSuspend[] {
  const limit = Math.min(lines.length, FILE_HEAD_SCAN_LINES)
  const head: ParsedSuspend[] = []
...
function violationHasInlineSuspend(violation: Violation, cache: SharedCache, rootDir: string): boolean {
  ...
  for (const p of parseFileHeadFileSuspends(lines)) {
    if (suspendCoversViolation(p, violation)) return true
  }
 ...
 */
function applyInlineSuspendMarkers(report: CheckReport, rootDir: string, cache: SharedCache): void {
