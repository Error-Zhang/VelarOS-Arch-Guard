...
import { computeFingerprint,type Violation, violationKey } from './violation'
...
class Baseline {
  ...
  private readonly slots: ReadonlyMap<string, WaiverSlot>
  /** `checkId::ruleId::fingerprint` → 该指纹下的全部条目，用于识别「撞上了但内容不符」。 */
  private readonly byFingerprint: ReadonlyMap<string, readonly BaselineEntry[]>

  constructor(entries: readonly BaselineEntry[]) {
    const slots = new Map<string, { entries: BaselineEntry[]; capacity: number | undefined }>()
   ...
   */
  public openScan(): BaselineScan {
    return new BaselineScan(this.slots, this.byFingerprint)
  }

  /** 指纹相同的条目是否多于一条——说明该指纹本身没有区分力。 */
  public get collidingFingerprintCount(): number {
    let count = 0
    for (const bucket of this.byFingerprint.values()) {
      if (bucket.length > 1) count += 1
    }
    return count
  }

  /** 全部条目（含未命中），供 baseline 命令做合并写回。 */
  public get allEntries(): BaselineEntry[] {
    const result: BaselineEntry[] = []
    for (const slot of this.slots.values()) result.push(...slot.entries)
    return result
  }
}
...
class BaselineScan {
  ...
  private readonly mismatches: BaselineContentMismatch[] = []
  ...
  public isWaived(violation: Violation): boolean {
    ...
    if (exact && this.consume(exactKey, exact)) return true
    // 旧条目（≤0.2.x 写下、没有摘要）：沿用只比指纹、不计数的口径，旧基线零迁移可用。
    const legacyKey = `${key}::*`
    const legacy = this.slots.get(legacyKey)
    if (legacy && this.consume(legacyKey, legacy)) return true
  ...
  private consume(waiverKey: string, slot: WaiverSlot): boolean {
    const used = this.used.get(waiverKey) ?? 0
    if (slot.capacity !== undefined && used >= slot.capacity) return false
  ...
  public get staleEntries(): BaselineEntry[] {
    ...
    for (const [waiverKey, slot] of this.slots) {
      if ((this.used.get(waiverKey) ?? 0) === 0) result.push(...slot.entries)
    }
    return result
  }
  ...
  public get contentMismatches(): readonly BaselineContentMismatch[] {
    return this.mismatches
  }
}

/** 配额解析：缺席 / 非法值一律当 1，不当无限——身份字段出错时必须收紧而不是放宽。 */
function normalizeEntryCount(count: number | undefined): number {
  if (typeof count !== 'number' || !Number.isInteger(count) || count < 1) return 1
  return count
}

/** 写回时省略 `count: 1`，让绝大多数条目与 0.2.x 的形状逐字一致。 */
function withEntryCount(entry: BaselineEntry, count: number): BaselineEntry {
  if (count <= 1) {
 ...
 * 也就是 `update` 不再幂等。
 */
function collapseEntriesByWaiverKey(entries: readonly BaselineEntry[]): BaselineEntry[] {
  const order: string[] = []
  const byKey = new Map<string, { entry: BaselineEntry; count: number }>()
  for (const entry of entries) {
...
/** 一条条目当前占用的配额（缺省 1）。 */
function entryWaiverCount(entry: BaselineEntry): number {
 ...
 */
function waiverKeyFromBaselineEntry(entry: BaselineEntry): string {
 ...
 */
function normalizeMessageForContentDigest(message: string): string {
  return message.replace(/^(\S*?):(\d+)(?::(\d+))?:\s?/, '$1: ')
}

/** 计算违规的内容摘要（与行号无关）。 */
function contentDigestForMessage(message: string): string {
  return computeFingerprint(['content:v1', normalizeMessageForContentDigest(message)])
}

function contentDigestOfViolation(violation: Violation): string {
...
function readBaselineEntries(filePath: string): BaselineEntry[] {
...
function baselineEntryFromViolation(violation: Violation): BaselineEntry {
...
function writeBaselineEntriesFile(filePath: string, entries: readonly BaselineEntry[]): void {
  ...
  if (existsSync(filePath) && readFileSync(filePath, 'utf-8') === next) return
  writeFileAtomically(filePath, next)
}

/** 序列化成最终文件内容（按豁免键排序，便于 git diff 审查）。 */
function serializeBaselineEntries(entries: readonly BaselineEntry[]): string {
  const sortedEntries = [...entries].sort((a, b) =>
    waiverKeyFromBaselineEntry(a).localeCompare(waiverKeyFromBaselineEntry(b))
  )
