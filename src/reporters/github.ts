    ...
    for (const report of aggregate.reports) {
      for (const section of report.failedSections) {
        for (const violation of section.violations) {
          ...
          const parts: string[] = []
          if (violation.file) parts.push(`file=${violation.file}`)
          if (violation.line !== undefined) parts.push(`line=${violation.line}`)
          if (violation.column !== undefined) parts.push(`col=${violation.column}`)
...
function severityToCommand(severity: SeverityLevel): 'error' | 'warning' | 'notice' {
