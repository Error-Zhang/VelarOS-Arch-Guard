...
import { expressionsTextEqual, matchNonBlankStringPositiveAndPair } from './nonBlankStringGuardChains.js';
...
const preferIsNonBlankStringGuard = defineCheck({
    ...
    description: '将 isString(x) 或 typeof x === string（含 ==）与 !!x.trim()（或 x.trim() / Boolean(x.trim())，顺序任意）合并为 isNonBlankString(x)；可吸收紧前同参 truthy 守卫。',
    verifies: [
...
function planNonBlankAndReplacement(parts, sourceFile) {
...
function unwrapParensExpr(expr) {
    let e = expr;
...
function formatNonBlankOperand(expression, sourceFile) {
...
//# sourceMappingURL=preferIsNonBlankStringGuard.js.map