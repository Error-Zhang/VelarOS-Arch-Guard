...
const forbidUndefinedCoalescing = defineCheck({
    ...
    description: '禁止 `expr ?? undefined` 内联缺席归一化（§12.6）；缺席语义由 contract 表达（`?:` 或 `Nullable<T>`）。',
    verifies: [
        ...
        for (const info of collectCodeStyleFiles(context)) {
            ...
            walk(sourceFile, (node) => {
                ...
                if (node.operatorToken.kind !== ts.SyntaxKind.QuestionQuestionToken)
                    return;
                if (!isUndefinedExpression(unwrapParensExpression(node.right)))
                    return;
...
//# sourceMappingURL=forbidUndefinedCoalescing.js.map