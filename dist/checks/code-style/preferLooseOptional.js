        ...
        for (const info of collectCodeStyleFiles(context, { runtimeOnly: false })) {
            ...
            walk(sourceFile, (node) => {
                ...
                if (!ts.isUnionTypeNode(node))
                    return;
                if (isDefiningContractType(node))
                    return;
                ...
                if (!hasNullish || !hasValue)
                    return;
                if (hasUndefined && !hasNull && !shouldReportUndefinedUnion(node))
                    return;
...
function isDefiningContractType(node) {
    const parent = node.parent;
    return ts.isTypeAliasDeclaration(parent) && ContractTypeNames.has(parent.name.text);
}
function isOptionalNullableContract(node) {
    if (!ts.isTypeReferenceNode(node) || node.typeName.getText() !== 'Nullable')
        return false;
    const parent = node.parent;
    return ((ts.isPropertySignature(parent) ||
...
function flattenUnionParts(node) {
    const result = [];
    for (const part of node.types) {
        const unwrapped = unwrapTypeNode(part);
        if (ts.isUnionTypeNode(unwrapped))
            result.push(...flattenUnionParts(unwrapped));
        else
            result.push(unwrapped);
    }
    return result;
}
function isNullishTypePart(node) {
    return isNullTypePart(node) || isUndefinedTypePart(node);
}
function isNullTypePart(node) {
