...
function maxIfNestingDepth(node) {
    let max = 0;
    const visit = (current, depth) => {
        if (ts.isIfStatement(current)) {
            ...
            if (current.elseStatement)
                visit(current.elseStatement, depth); // else 不累加
            return;
        }
...
function describeFunctionOwner(sourceFile, node) {
    if (ts.isFunctionDeclaration(node) && node.name)
        return `function ${node.name.text}`;
    if (ts.isMethodDeclaration(node)) {
        const className = ts.isClassDeclaration(node.parent) && node.parent.name ? node.parent.name.text : '<class>';
...
//# sourceMappingURL=preferEarlyReturn.js.map