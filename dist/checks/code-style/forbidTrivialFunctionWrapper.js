...
const forbidTrivialFunctionWrapper = defineCheck({
    ...
    description: '禁止仅透传参数的一行转发函数（export function 与任意 const 箭头转发）；直接调用底层实现或 `export { x as y } from`。async / 解构参数 / rest 保守跳过。',
    verifies: [
            ...
            walk(sourceFile, (node) => {
                if (ts.isFunctionDeclaration(node) && node.name && node.body) {
                    ...
                    if (mods & (ts.ModifierFlags.Export | ts.ModifierFlags.Default)) {
                        reportIfTrivialForwarder(section, info.relativePath, sourceFile, node, node.name.text, importedValueNames);
                    }
                    return;
                }
                if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name) && node.initializer) {
                    if (ts.isArrowFunction(node.initializer) || ts.isFunctionExpression(node.initializer)) {
                        reportIfTrivialForwarder(section, info.relativePath, sourceFile, node.initializer, node.name.text, importedValueNames);
                    }
                    return;
                }
                if (ts.isExportAssignment(node) && !node.isExportEquals && node.expression) {
                    ...
                    if (ts.isArrowFunction(expr) || ts.isFunctionExpression(expr)) {
                        reportIfTrivialForwarder(section, info.relativePath, sourceFile, expr, 'default', importedValueNames);
                    }
                }
            });
        }
...
function functionLikeIsAsync(fn) {
    const mod = fn.modifiers;
    return mod?.some((m) => m.kind === ts.SyntaxKind.AsyncKeyword) ?? false;
}
function reportIfTrivialForwarder(section, relativePath, sourceFile, fn, declaredName, importedValueNames) {
    if (functionLikeIsAsync(fn))
        return;
    const body = fn.body;
    if (!body)
        return;
    ...
    if (!call)
        return;
    if (isBoundaryAliasCall(call, importedValueNames))
        return;
    if (!argsMatchParamsInOrder(call.arguments, paramNames))
        return;
...
function collectImportedValueNames(sourceFile) {
    const names = new Set();
...
function isBoundaryAliasCall(call, importedValueNames) {
...
function collectSimpleBindingParamNames(parameters) {
    const out = [];
...
function argsMatchParamsInOrder(args, paramNames) {
    if (args.length !== paramNames.length)
        return false;
    for (let i = 0; i < paramNames.length; i++) {
        const arg = args[i];
        if (!ts.isIdentifier(arg) || arg.text !== paramNames[i])
            return false;
    }
    return true;
}
export { forbidTrivialFunctionWrapper };
//# sourceMappingURL=forbidTrivialFunctionWrapper.js.map