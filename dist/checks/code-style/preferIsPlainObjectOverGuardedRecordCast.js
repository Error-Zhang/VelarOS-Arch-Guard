...
const preferIsPlainObjectOverGuardedRecordCast = defineCheck({
    ...
    description: '`isObject(x)` 守卫后又写 `x as Record<string, unknown>` 才能读字段的，改用 `isPlainObject(x)`：守卫本身就收窄成 Record，断言可整条删掉。',
    verifies: [
        ...
        for (const info of collectCodeStyleFiles(context)) {
            ...
            walk(sourceFile, (node) => {
                ...
                if (!ts.isAsExpression(node) && !ts.isTypeAssertionExpression(node))
                    return;
                if (!isUnknownRecordType(node.type))
                    return;
                const target = referenceTextOf(node.expression);
                if (target)
                    assertions.push({ target, node });
            });
            for (const assertion of assertions) {
                if (!guardedTargets.has(assertion.target))
                    continue;
                ...
                section.add({
                    ...
                    line,
                    message: `${info.relativePath}:${line}: \`${assertion.target}\` 被 isObject 守卫后仍断言成 ` +
...
function isIsObjectCall(node) {
    const callee = node.expression;
    if (ts.isIdentifier(callee))
        return callee.text === 'isObject';
...
function isUnknownRecordType(typeNode) {
...
function referenceTextOf(node) {
    if (!node)
        return null;
    if (ts.isParenthesizedExpression(node))
        return referenceTextOf(node.expression);
    if (ts.isIdentifier(node))
        return node.text;
    if (ts.isThisTypeNode(node) || node.kind === ts.SyntaxKind.ThisKeyword)
        return 'this';
...
//# sourceMappingURL=preferIsPlainObjectOverGuardedRecordCast.js.map