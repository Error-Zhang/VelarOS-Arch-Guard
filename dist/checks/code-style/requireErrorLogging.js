        ...
        for (const info of collectCodeStyleFiles(context)) {
            ...
            walk(sourceFile, (node) => {
                if (!ts.isCatchClause(node))
                    return;
                if (hasInlineSuppression(sourceFile, node))
                    return;
                const catchParamName = readCatchParameterName(node);
                if (isCatchHandled(sourceFile, node.block, catchParamName))
                    return;
...
function readCatchParameterName(clause) {
    if (!clause.variableDeclaration)
        return undefined;
    const name = clause.variableDeclaration.name;
    return ts.isIdentifier(name) ? name.text : undefined;
}
function hasInlineSuppression(sourceFile, clause) {
...
function isCatchHandled(sourceFile, block, catchParamName) {
    let handled = false;
    const visit = (node) => {
        if (handled)
            return;
...
function returnExpressionReferences(node, name) {
    let found = false;
    const visit = (n) => {
        if (found)
            return;
...
function isErrorReportingCall(sourceFile, call) {
    const callText = call.expression.getText(sourceFile);
    if (ErrorReportingTextPatterns.some((pattern) => pattern.test(callText)))
        return true;
    const calleeName = readCalleeName(call.expression);
    if (calleeName) {
        if (FailureCallExactNames.has(calleeName))
            return true;
        if (FailureCallNamePatterns.some((pattern) => pattern.test(calleeName)))
            return true;
    }
    return call.arguments.some((arg) => ts.isObjectLiteralExpression(arg) && isErrorResultObject(arg));
}
function isErrorResultAssignment(node) {
    return (ts.isBinaryExpression(node) &&
        node.operatorToken.kind === ts.SyntaxKind.EqualsToken &&
        ts.isObjectLiteralExpression(node.right) &&
        isErrorResultObject(node.right));
}
function isErrorResultObject(obj) {
    for (const property of obj.properties) {
        ...
        if (!name)
            continue;
        if (ErrorPayloadNamePattern.test(name))
            return true;
        if (name === 'ok' && property.initializer.kind === ts.SyntaxKind.FalseKeyword)
            return true;
        if ((name === 'status' || name === 'state') &&
            ts.isStringLiteralLike(property.initializer) &&
            (property.initializer.text === 'failed' || property.initializer.text === 'error'))
            return true;
        if ((name === 'kind' || name === 'tone') &&
            ts.isStringLiteralLike(property.initializer) &&
            property.initializer.text === 'error')
            return true;
        if (ts.isObjectLiteralExpression(property.initializer) && isErrorResultObject(property.initializer))
            return true;
    }
    return false;
}
...
function readCalleeName(expression) {
...
//# sourceMappingURL=requireErrorLogging.js.map