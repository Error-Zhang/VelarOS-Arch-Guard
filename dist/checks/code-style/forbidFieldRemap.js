...
const DataCarrierSourcePattern = /^(?:input|args|config|options|payload|record|row|data|request|response|event\.payload)$/;
const DataBoundaryFilePatterns = [
    /\.d\.ts$/,
    /\.generated\./,
    /\/generated\//,
    /\/i18n\/messages\//,
...
const forbidFieldRemap = defineCheck({
    ...
    run({ context, report }) {
        ...
        for (const info of collectCodeStyleFiles(context)) {
            if (DataBoundaryFilePatterns.some((pattern) => pattern.test(info.relativePath)))
                continue;
            ...
            walk(sourceFile, (node) => {
                ...
                for (const [sourceText, propertyNames] of sourceGroups) {
                    if (!DataCarrierSourcePattern.test(sourceText))
                        continue;
                    if (propertyNames.length < SameNameFieldThreshold)
                        continue;
...
function readSameNameAssignment(sourceFile, property) {
    ...
    if (!ts.isPropertyAccessExpression(property.initializer))
        return undefined;
    if (property.initializer.name.text !== name)
        return undefined;
    return {
        propertyName: name,
...
//# sourceMappingURL=forbidFieldRemap.js.map