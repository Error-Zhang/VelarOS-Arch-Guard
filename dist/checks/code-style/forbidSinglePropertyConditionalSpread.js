...
const forbidSinglePropertyConditionalSpread = defineCheck({
    ...
    description: '单个可选字段不要写成条件对象 spread；使用全局 toOptional(value) 表达 `?? undefined` 语义。',
    verifies: [
...
function readSinglePropertyConditionalSpread(node, sourceFile) {
    ...
    if (whenTrue?.kind === 'single' && whenFalse?.kind === 'empty')
        return buildReplacement(sourceFile, expression.condition, whenTrue.property, false);
    if (whenFalse?.kind === 'single' && whenTrue?.kind === 'empty')
        return buildReplacement(sourceFile, expression.condition, whenFalse.property, true);
    return undefined;
}
...
function buildReplacement(sourceFile, condition, property, invertCondition) {
    ...
    if (!invertCondition && conditionText === value)
        return {
            propertyName,
...
function readNonNullishGuardSelf(condition, value, sourceFile) {
    const call = unwrapParensExpression(condition);
    if (!ts.isCallExpression(call))
        return undefined;
    const callee = unwrapParensExpression(call.expression);
    if (!ts.isIdentifier(callee) || !NonNullishGuardNames.has(callee.text))
        return undefined;
...
function readObjectLiteralShape(expression) {
    ...
    if (ts.isPropertyAssignment(property) || ts.isShorthandPropertyAssignment(property))
        return { kind: 'single', property };
    return undefined;
}
function readPropertyName(property, sourceFile) {
...
function readPropertyValue(property, sourceFile) {
    if (ts.isShorthandPropertyAssignment(property))
        return property.name.getText(sourceFile);
    if (ts.isPropertyAssignment(property))
