...
const forbidRawTimers = defineCheck({
    ...
    description: '运行时禁止直调 setTimeout/clearTimeout/setInterval/requestAnimationFrame 等原生计时 API。',
    verifies: [
        ...
        for (const info of collectCodeStyleFiles(context)) {
            ...
            walk(sourceFile, (node) => {
                ...
                section.add({
                    ...
                    suggestion: 'Replace with TimerScope.after/every/nextFrame/sleep/withTimeout, or useTimerScope in React.',
                });
            });
        }
    },
});
function readTimerCallName(sourceFile, expression) {
    if (ts.isIdentifier(expression))
        return TimerNames.has(expression.text) ? expression.text : undefined;
    if (!ts.isPropertyAccessExpression(expression))
        return undefined;
    if (!TimerNames.has(expression.name.text))
        return undefined;
...
//# sourceMappingURL=forbidRawTimers.js.map