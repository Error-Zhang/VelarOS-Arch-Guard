...
import { flattenAndOperands, flattenOrChainOperands, matchPlainObjectPositiveRawTriple, matchRejectPlainObjectOrDeMorganPair, matchRejectPlainObjectOrTriple, topOfAndChain, topOfOrChain, } from './plainObjectRawAndTriple.js';
/**
...
const preferIsPlainObjectOverObjectArrayGuard = defineCheck({
    ...
    description: '将 `typeof`/null/array 的 `&&` 三连或 `isObject`+`!isArray` 合并为 `isPlainObject`；将 **拒 plain** 的 `||` **三连**（falsy/nullish、非 object、数组三槽）或 **De Morgan 二连**（`!isObject`+`isArray`、`typeof`+`Array.isArray` 等）合并为 `!isPlainObject`；保留链上其它子式。细则见源码 `plainObjectRawAndTriple.ts` 顶部注释。',
    verifies: [
...
function planPlainObjectReplacement(parts, sourceFile) {
    ...
    for (let i = 0; i < n - 1; i += 1) {
        ...
        if (!objCall || !arrArg || !expressionsTextEqual(objCall.arg, arrArg, sourceFile))
            continue;
        ...
        while (start >= 0) {
            ...
            if (!guard || !isRedundantTruthyGuardOn(guard, objCall.arg, sourceFile))
                break;
            start -= 1;
        }
...
function planRejectPlainObjectReplacement(parts, sourceFile) {
    for (let start = 0; start <= parts.length - 3; start += 1) {
...
function readGlobalUnaryCall(expr, name, _sourceFile) {
    const u = unwrapParens(expr);
    if (!ts.isCallExpression(u) || u.arguments.length !== 1)
        return undefined;
    if (!ts.isIdentifier(u.expression) || u.expression.text !== name)
        return undefined;
...
function readNegatedGlobalCallArg(expr, name, _sourceFile) {
    const u = unwrapParens(expr);
    ...
    if (!ts.isCallExpression(inner) || inner.arguments.length !== 1)
        return undefined;
    if (!ts.isIdentifier(inner.expression) || inner.expression.text !== name)
        return undefined;
...
function formatPlainObjectOperand(expression, sourceFile) {
...
//# sourceMappingURL=preferIsPlainObjectOverObjectArrayGuard.js.map