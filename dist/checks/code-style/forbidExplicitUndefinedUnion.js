...
import { collectCodeStyleFiles, columnOf, getCachedSourceFile, lineOf, snippetOf, walk } from './_shared.js';
/**
...
const forbidExplicitUndefinedUnion = defineCheck({
    ...
    description: '非可选 prop/param 声明位 `T | undefined` 计数棘轮（§12.6）；缺席用 `Nullable<T>`（null），可选用 `?:`。',
    verifies: [
        ...
        for (const info of collectCodeStyleFiles(context)) {
            ...
            walk(sourceFile, (node) => {
                if (!ts.isUnionTypeNode(node))
                    return;
                if (!node.types.some((member) => member.kind === ts.SyntaxKind.UndefinedKeyword))
                    return;
                if (!isNonOptionalDeclarationPosition(node))
                    return;
 ...
 */
function isNonOptionalDeclarationPosition(union) {
    const owner = union.parent;
    if (ts.isPropertySignature(owner) ||
...
//# sourceMappingURL=forbidExplicitUndefinedUnion.js.map