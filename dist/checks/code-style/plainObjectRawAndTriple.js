...
function topOfAndChain(node) {
    let current = node;
    while (ts.isBinaryExpression(current.parent) &&
...
function flattenOrChainOperands(expr) {
    const u = unwrapParens(expr);
    if (ts.isBinaryExpression(u) && u.operatorToken.kind === ts.SyntaxKind.BarBarToken)
        return [...flattenOrChainOperands(u.left), ...flattenOrChainOperands(u.right)];
    return [u];
}
/** 自任意子孙 `&&` 结点找到顶层 `&&` 链根。 */
...
function flattenAndOperands(expr) {
    const u = unwrapParens(expr);
    if (ts.isBinaryExpression(u) && u.operatorToken.kind === ts.SyntaxKind.AmpersandAmpersandToken)
        return [...flattenAndOperands(u.left), u.right];
    return [u];
}
function tryParseTypeofEqualsObjectOperand(node) {
    if (!ts.isBinaryExpression(node))
        return undefined;
    const op = node.operatorToken.kind;
    if (op !== ts.SyntaxKind.EqualsEqualsEqualsToken && op !== ts.SyntaxKind.EqualsEqualsToken)
        return undefined;
    if (ts.isTypeOfExpression(node.left) && ts.isStringLiteral(node.right) && node.right.text === 'object')
        return node.left.expression;
    if (ts.isTypeOfExpression(node.right) && ts.isStringLiteral(node.left) && node.left.text === 'object')
        return node.right.expression;
    return undefined;
}
function tryParseTypeofInequalityObjectOperand(node) {
    if (!ts.isBinaryExpression(node))
        return undefined;
    const op = node.operatorToken.kind;
...
function tryParseTypeofEqualityUndefinedOperand(node) {
    if (!ts.isBinaryExpression(node))
        return undefined;
    const op = node.operatorToken.kind;
    if (op !== ts.SyntaxKind.EqualsEqualsEqualsToken && op !== ts.SyntaxKind.EqualsEqualsToken)
...
function tryParseStrictInequalityNullOperand(node) {
    if (!ts.isBinaryExpression(node))
        return undefined;
...
function isBareArrayIsArrayCall(node) {
    if (node.arguments.length !== 1)
        return false;
    const ex = node.expression;
    if (!ts.isPropertyAccessExpression(ex) || ex.name.text !== 'isArray')
        return false;
    return ts.isIdentifier(ex.expression) && ex.expression.text === 'Array';
}
/** `!isArray(x)` */
function readNegatedGlobalIsArrayArg(expr) {
    const u = unwrapParens(expr);
    ...
    if (!ts.isCallExpression(inner) || inner.arguments.length !== 1)
        return undefined;
    if (!ts.isIdentifier(inner.expression) || inner.expression.text !== 'isArray')
        return undefined;
...
function readNegatedArrayOperand(expr) {
    ...
    if (!ts.isPrefixUnaryExpression(u) || u.operator !== ts.SyntaxKind.ExclamationToken)
        return undefined;
    const inner = unwrapParens(u.operand);
    if (!ts.isCallExpression(inner) || !isBareArrayIsArrayCall(inner))
        return undefined;
    const arg = inner.arguments[0];
    if (!arg)
        return undefined;
...
function readNegatedGlobalUnaryCallArg(expr, fn) {
    const u = unwrapParens(expr);
    if (!ts.isPrefixUnaryExpression(u) || u.operator !== ts.SyntaxKind.ExclamationToken)
        return undefined;
    const inner = unwrapParens(u.operand);
    if (!ts.isCallExpression(inner) || inner.arguments.length !== 1)
        return undefined;
    if (!ts.isIdentifier(inner.expression) || inner.expression.text !== fn)
        return undefined;
    const arg = inner.arguments[0];
    if (!arg)
        return undefined;
...
function extractEqNullOrUndefinedSubject(expr) {
    const u = unwrapParens(expr);
    ...
    if (op !== ts.SyntaxKind.EqualsEqualsToken && op !== ts.SyntaxKind.EqualsEqualsEqualsToken)
        return undefined;
    if (u.left.kind === ts.SyntaxKind.NullKeyword || u.left.kind === ts.SyntaxKind.UndefinedKeyword)
        return unwrapParens(u.right);
    if (u.right.kind === ts.SyntaxKind.NullKeyword || u.right.kind === ts.SyntaxKind.UndefinedKeyword)
        return unwrapParens(u.left);
    return undefined;
}
function extractIsNullOrUndefinedCallSubject(expr) {
    const u = unwrapParens(expr);
    if (!ts.isCallExpression(u) || u.arguments.length !== 1)
        return undefined;
    if (!ts.isIdentifier(u.expression))
        return undefined;
    if (u.expression.text !== 'isNull' && u.expression.text !== 'isUndefined')
        return undefined;
 ...
 */
function extractFalsySlotSubject(expr, _sourceFile) {
    const u = unwrapParens(expr);
    const nullUndefCall = extractIsNullOrUndefinedCallSubject(u);
    if (nullUndefCall)
        return nullUndefCall;
...
function readNotObjectAndNotNullSubject(expr, sourceFile) {
    const u = unwrapParens(expr);
    if (!ts.isBinaryExpression(u) || u.operatorToken.kind !== ts.SyntaxKind.AmpersandAmpersandToken)
        return undefined;
    ...
    if (lo && rn && expressionsTextEqual(lo, rn, sourceFile))
        return lo;
    const ln = readNegatedGlobalUnaryCallArg(l, 'isNull');
    const ro = readNegatedGlobalUnaryCallArg(r, 'isObject');
    if (ln && ro && expressionsTextEqual(ln, ro, sourceFile))
        return ln;
    return undefined;
}
function extractNotObjectSlotSubject(expr, sourceFile) {
    const u = unwrapParens(expr);
...
function readPositiveArrayArg(expr) {
    const u = unwrapParens(expr);
    if (!ts.isCallExpression(u) || u.arguments.length !== 1)
        return undefined;
    const arg = u.arguments[0];
    if (!arg)
        return undefined;
    if (ts.isIdentifier(u.expression) && u.expression.text === 'isArray')
        return arg;
    if (isBareArrayIsArrayCall(u))
        return arg;
    return undefined;
}
const REJECT_PLAIN_OR_ROLE_PERMUTATIONS = [
...
function extractSubjectForRejectOrRole(part, role, sourceFile) {
    switch (role) {
        case 'falsy':
...
function matchRejectPlainObjectOrTriple(parts, sourceFile) {
    if (parts.length !== 3)
        return undefined;
    for (const perm of REJECT_PLAIN_OR_ROLE_PERMUTATIONS) {
        ...
        if (!ok || subjects.length !== 3)
            continue;
        const [s0, s1, s2] = subjects;
        if (!s0 || !s1 || !s2)
            continue;
        if (expressionsTextEqual(s0, s1, sourceFile) && expressionsTextEqual(s0, s2, sourceFile))
            return s0;
    }
    return undefined;
}
function matchRejectPlainObjectOrDeMorganPair(parts, sourceFile) {
    if (parts.length !== 2)
        return undefined;
    const first = parts[0];
...
function classifyRawAndPart(expr) {
    const u = unwrapParens(expr);
    const typeofSubj = tryParseTypeofEqualsObjectOperand(u);
    if (typeofSubj)
        return { kind: 'typeof-object', subj: typeofSubj };
    const notNullSubj = tryParseStrictInequalityNullOperand(u);
    if (notNullSubj)
        return { kind: 'not-null', subj: notNullSubj };
    const arrSubj = readNegatedArrayOperand(u);
    if (arrSubj)
        return { kind: 'not-array', subj: arrSubj };
    return undefined;
}
function matchPlainObjectPositiveRawTriple(parts, sourceFile) {
    if (parts.length !== 3)
        return undefined;
    const kinds = new Set();
    let subj;
    for (const p of parts) {
        ...
        else if (!expressionsTextEqual(subj, c.subj, sourceFile))
            return undefined;
    }
    if (kinds.size !== 3 || !subj)
        return undefined;
    return subj;
}
function readTypeofObjectStrictNotNullMerge(node, sourceFile) {
    ...
    if (left && rightNull && expressionsTextEqual(left, rightNull, sourceFile))
        return { expr: left };
    ...
    if (leftNull && rightObj && expressionsTextEqual(leftNull, rightObj, sourceFile))
        return { expr: leftNull };
    return undefined;
}
/** forbid-raw：`typeof&&!null` 合并会妨碍 isPlainObject 一步到位时跳过。 */
function shouldSkipMergeTypeofObjectNotNull(mergeNode, sourceFile) {
    if (!readTypeofObjectStrictNotNullMerge(mergeNode, sourceFile))
        return false;
    const top = topOfAndChain(mergeNode);
    const parts = flattenAndOperands(top);
    for (let i = 0; i <= parts.length - 3; i += 1) {
        if (!matchPlainObjectPositiveRawTriple(parts.slice(i, i + 3), sourceFile))
            continue;
        const a = parts[i];
        const b = parts[i + 1];
        if (mergeNode.getStart(sourceFile) === a.getStart(sourceFile) &&
            mergeNode.getEnd() === b.getEnd())
            return true;
    }
    return false;
}
function shouldSkipBareArrayIsArrayInPositivePlainTriple(call, sourceFile) {
    const bang = call.parent;
    if (!ts.isPrefixUnaryExpression(bang) || bang.operator !== ts.SyntaxKind.ExclamationToken)
        return false;
    ...
    for (let i = 0; i <= parts.length - 3; i += 1) {
        ...
        if (call.getStart(sourceFile) >= third.getStart(sourceFile) && call.getEnd() <= third.getEnd())
            return true;
    }
    return false;
}
/** forbid-raw：`Array.isArray` → `isArray`：在 **正** plain `&&` 三连的 `!Array.isArray` 内，或在 **拒** plain `||` 三连的 **正** `Array.isArray` 臂上时跳过（由 prefer 一次替换整链）。 */
function shouldSkipBareArrayIsArrayCall(call, sourceFile) {
    if (!isBareArrayIsArrayCall(call))
        return false;
    if (shouldSkipBareArrayIsArrayInPositivePlainTriple(call, sourceFile))
        return true;
    ...
    for (let i = 0; i <= orParts.length - 3; i += 1) {
        ...
        if (!subjWindow)
            continue;
        if (windowContainsArrayCall(slice, subjWindow, call, sourceFile))
            return true;
    }
...
function windowContainsArrayCall(slice, subj, call, sourceFile) {
    for (const part of slice) {
        const arrArg = readPositiveArrayArg(part);
        if (!arrArg || !expressionsTextEqual(arrArg, subj, sourceFile))
            continue;
        if (call.getStart(sourceFile) >= part.getStart(sourceFile) && call.getEnd() <= part.getEnd())
            return true;
    }
    return false;
}
/** forbid-raw：`typeof x === 'object'` 单独改 isObject 会破坏三连时跳过。 */
function shouldSkipTypeofObjectStrictEqBinary(node, sourceFile) {
    const op = node.operatorToken.kind;
    if (op !== ts.SyntaxKind.EqualsEqualsEqualsToken && op !== ts.SyntaxKind.EqualsEqualsToken)
        return false;
    if (!tryParseTypeofEqualsObjectOperand(unwrapParens(node)))
        return false;
...
function shouldSkipTypeofObjectNotEqForRejectTriple(node, sourceFile) {
    const op = node.operatorToken.kind;
    if (op !== ts.SyntaxKind.ExclamationEqualsEqualsToken && op !== ts.SyntaxKind.ExclamationEqualsToken)
        return false;
    if (!tryParseTypeofInequalityObjectOperand(unwrapParens(node)))
        return false;
    ...
    for (let i = 0; i <= orParts.length - 3; i += 1) {
        const slice = orParts.slice(i, i + 3);
        if (!matchRejectPlainObjectOrTriple(slice, sourceFile))
            continue;
        for (const part of slice) {
            if (part === node)
                return true;
    ...
    for (let i = 0; i <= orParts.length - 2; i += 1) {
        const slice = orParts.slice(i, i + 2);
        if (!matchRejectPlainObjectOrDeMorganPair(slice, sourceFile))
            continue;
        for (const part of slice) {
            if (part === node)
                return true;
...
//# sourceMappingURL=plainObjectRawAndTriple.js.map