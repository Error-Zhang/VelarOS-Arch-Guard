...
const forbidNullishChurn = defineCheck({
    ...
    description: '业务代码不要内联做 null/undefined 转换；归一到 `null` 时用全局 toNullable（extensions），其余在边界 helper 处理。',
    verifies: [
        ...
        for (const info of collectCodeStyleFiles(context)) {
            ...
            walk(sourceFile, (node) => {
                if (ts.isConditionalExpression(node)) {
                    ...
                    if (enclosingIife && readIsPresentNullIifeToNullable(enclosingIife, sourceFile))
                        return;
...
function readNullishFallback(node) {
...
function readNullishComparison(sourceFile, node) {
    if (!ts.isBinaryExpression(node))
        return undefined;
    const kind = node.operatorToken.kind;
    const isEqualityOp = kind === ts.SyntaxKind.EqualsEqualsToken || kind === ts.SyntaxKind.EqualsEqualsEqualsToken;
    const isInequalityOp = kind === ts.SyntaxKind.ExclamationEqualsToken ||
        kind === ts.SyntaxKind.ExclamationEqualsEqualsToken;
    if (!isEqualityOp && !isInequalityOp)
        return undefined;
...
function readIsPresentNullCoalesce(node, sourceFile) {
    if (getNullishKind(unwrapExpr(node.whenFalse)) !== 'null')
        return undefined;
    ...
    if (inner.getText(sourceFile) !== whenTrue.getText(sourceFile))
        return undefined;
    return { inner };
}
function readToOptionalTruthySelfTernary(call, sourceFile) {
...
function readToOptionalNumberGuardSelfTernary(call, sourceFile) {
    const ternary = readToOptionalUndefinedTernary(call);
    if (!ternary)
        return undefined;
    const condition = unwrapExpr(ternary.condition);
...
function readToOptionalUndefinedTernary(call) {
    ...
    if (!ts.isConditionalExpression(ternary))
        return undefined;
    if (getNullishKind(unwrapExpr(ternary.whenFalse)) !== 'undefined')
        return undefined;
    return ternary;
}
function enclosingIsPresentNullIifeCall(node) {
    let p = node.parent;
    ...
    if (!ts.isExpression(arrow.body))
        return undefined;
    if (unwrapExpr(arrow.body) !== node)
        return undefined;
    let outer = arrow.parent;
    while (ts.isParenthesizedExpression(outer))
        outer = outer.parent;
    if (!ts.isCallExpression(outer))
        return undefined;
    if (unwrapExpr(outer.expression) !== arrow)
        return undefined;
    if (outer.arguments.length !== 1)
        return undefined;
    return outer;
}
function readIsPresentNullIifeToNullable(call, sourceFile) {
    if (call.arguments.length !== 1)
        return undefined;
    const callee = unwrapExpr(call.expression);
    if (!ts.isArrowFunction(callee))
        return undefined;
    if (callee.parameters.length !== 1)
        return undefined;
    ...
    if (!ts.isIdentifier(innerId) || innerId.text !== param.name.text)
        return undefined;
...
function readNullishTernaryChurn(sourceFile, node) {
    if (!ts.isConditionalExpression(node))
        return undefined;
    ...
    if (!nullishBranchKind)
        return undefined;
    if (nullishBranchKind === comparison.nullishKind)
        return undefined;
    if (nonNullishBranch.getText(sourceFile) !== comparison.expressionText)
        return undefined;
    return { from: comparison.nullishKind, to: nullishBranchKind };
}
export { forbidNullishChurn };
//# sourceMappingURL=forbidNullishChurn.js.map