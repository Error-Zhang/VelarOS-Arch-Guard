...
const preferMeaningfulUseMemo = defineCheck({
    ...
    description: '（建议）useMemo 应用于昂贵计算或明确的引用稳定性边界；仅组装轻量对象/数组且依赖较长时提示改为直接构造。',
    verifies: [
        ...
        for (const info of collectCodeStyleFiles(context, { frontendOnly: true })) {
            ...
            walk(sourceFile, (node) => {
                ...
                section.add({
                    ...
                    suggestion: '删除 useMemo 与依赖数组，保留对象/数组本体；若确实需要引用稳定性，请用 @arch-guard:suspend 标记暂缓并说明 consumer 边界。',
                });
            });
        }
    },
});
function readLowValueUseMemo(node) {
    if (!ts.isCallExpression(node))
        return undefined;
    if (!isUseMemoCallee(node.expression))
        return undefined;
    const returned = readReturnedExpression(node.arguments[0]);
    if (!returned)
        return undefined;
    const dependencyCount = readDependencyCount(node.arguments[1]);
    if (dependencyCount < MinimumDependencyCount)
        return undefined;
    const literal = unwrapParensExpression(returned);
    if (ts.isObjectLiteralExpression(literal))
        return readLiteralMatch(literal, 'object', dependencyCount);
    if (ts.isArrayLiteralExpression(literal))
        return readLiteralMatch(literal, 'array', dependencyCount);
    return undefined;
}
function isUseMemoCallee(expression) {
    const unwrappedExpression = unwrapParensExpression(expression);
    if (ts.isIdentifier(unwrappedExpression))
        return unwrappedExpression.text === 'useMemo';
    return (ts.isPropertyAccessExpression(unwrappedExpression) &&
        unwrappedExpression.name.text === 'useMemo');
}
function readReturnedExpression(factory) {
    if (!factory)
        return undefined;
    const unwrappedFactory = unwrapParensExpression(factory);
    if (ts.isArrowFunction(unwrappedFactory))
        return readConciseBodyExpression(unwrappedFactory.body);
    if (ts.isFunctionExpression(unwrappedFactory))
        return readSingleReturnExpression(unwrappedFactory.body);
    return undefined;
}
function readConciseBodyExpression(body) {
    if (ts.isBlock(body))
        return readSingleReturnExpression(body);
    return body;
}
function readSingleReturnExpression(block) {
...
function readDependencyCount(dependencies) {
    if (!dependencies)
        return 0;
    const unwrappedDependencies = unwrapParensExpression(dependencies);
    if (!ts.isArrayLiteralExpression(unwrappedDependencies))
        return 0;
    return unwrappedDependencies.elements.length;
}
function readLiteralMatch(literal, literalKind, dependencyCount) {
    if (containsPotentiallyExpensiveExpression(literal))
        return undefined;
    return { dependencyCount, literalKind };
}
function containsPotentiallyExpensiveExpression(node) {
    let found = false;
    const visit = (current) => {
        if (found)
            return;
...
function isPotentiallyExpensiveExpression(node) {
...
//# sourceMappingURL=preferMeaningfulUseMemo.js.map