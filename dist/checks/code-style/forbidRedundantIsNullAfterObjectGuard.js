...
const forbidRedundantIsNullAfterObjectGuard = defineCheck({
    ...
    description: '在 `||` 链中，若先前子式已保证 `x` 非 null，则 `!isObject(x) && !isNull(x)` 可化为 `!isObject(x)`。',
    verifies: [
...
function fixDropIsNullConjunct(relativePath, sourceFile, andExpr, isNullOn) {
    return (ctx) => {
...
function flattenOrOperands(expr) {
    const u = unwrapParens(expr);
    if (ts.isBinaryExpression(u) && u.operatorToken.kind === ts.SyntaxKind.BarBarToken)
        return [...flattenOrOperands(u.left), ...flattenOrOperands(u.right)];
    return [u];
}
function impliesNotNullWhenPriorOrOperandIsFalsy(part, target, sourceFile) {
    const p = unwrapParens(part);
    if (ts.isPrefixUnaryExpression(p) && p.operator === ts.SyntaxKind.ExclamationToken)
        return p.operand.getText(sourceFile).trim() === target;
    if (ts.isBinaryExpression(p)) {
        const op = p.operatorToken.kind;
        if (op !== ts.SyntaxKind.EqualsEqualsToken && op !== ts.SyntaxKind.EqualsEqualsEqualsToken)
            return false;
        if (p.right.kind === ts.SyntaxKind.NullKeyword)
            return p.left.getText(sourceFile).trim() === target;
        if (p.left.kind === ts.SyntaxKind.NullKeyword)
            return p.right.getText(sourceFile).trim() === target;
        return false;
    }
    if (ts.isCallExpression(p) && p.arguments.length === 1) {
        ...
        if (ts.isIdentifier(callee) && callee.text === 'isNull')
            return arg0.getText(sourceFile).trim() === target;
    }
    return false;
}
function readNotIsObjectAndNotIsNull(expr, sourceFile) {
    const b = unwrapParens(expr);
    if (!ts.isBinaryExpression(b) || b.operatorToken.kind !== ts.SyntaxKind.AmpersandAmpersandToken)
        return undefined;
    ...
    if (leftN && rightN && leftN === rightN)
        return { targetExprText: leftN, andExpr: b, isNullOn: 'right' };
    ...
    if (leftM && rightM && leftM === rightM)
        return { targetExprText: leftM, andExpr: b, isNullOn: 'left' };
    return undefined;
}
function readNotGlobalCallArg(expr, fn, sourceFile) {
    const u = unwrapParens(expr);
    ...
    if (!ts.isIdentifier(callee) || callee.text !== fn)
        return undefined;
    return arg0.getText(sourceFile).trim();
}
export { forbidRedundantIsNullAfterObjectGuard };
//# sourceMappingURL=forbidRedundantIsNullAfterObjectGuard.js.map