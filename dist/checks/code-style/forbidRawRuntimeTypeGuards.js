...
import { shouldSkipTypeofNumberEqForFiniteAndPair, shouldSkipTypeofNumberEqForNumberOrNullTernary, shouldSkipTypeofNumberNeForFiniteOrPair, } from './finiteNumberGuardChains.js';
import { CodeStyleFixPhase } from './fixPhases.js';
import { shouldSkipTypeofStringEqForNonBlankAndPair, shouldSkipTypeofStringEqForTrimmedStringOrEmptyTernary, } from './nonBlankStringGuardChains.js';
import { shouldSkipBareArrayIsArrayCall, shouldSkipMergeTypeofObjectNotNull, shouldSkipTypeofObjectNotEqForRejectTriple, shouldSkipTypeofObjectStrictEqBinary, } from './plainObjectRawAndTriple.js';
/**
...
const forbidRawRuntimeTypeGuards = defineCheck({
    ...
    description: '手写 `typeof` 与 string literal 比较（object 合并形态除外由本规则收拢为 isObject）、`Array.isArray` 应改为全局 isString/isNumber/…/isArray/isObject。',
    verifies: [
        ...
        for (const info of collectCodeStyleFiles(context)) {
            ...
            walk(sourceFile, (node) => {
                if (ts.isBinaryExpression(node) &&
                    ...
                    if (merged) {
                        if (shouldSkipMergeTypeofObjectNotNull(node, sourceFile))
                            return;
                ...
                if (ts.isCallExpression(node) && isBareArrayIsArrayCall(node)) {
                    if (shouldSkipBareArrayIsArrayCall(node, sourceFile))
                        return;
                ...
                if (op !== ts.SyntaxKind.EqualsEqualsEqualsToken &&
                    ...
                    return;
                if (isUnderTypeofObjectMerge(node))
                    return;
                const typeofPrim = readTypeofComparedToNonUndefinedString(node);
                if (!typeofPrim)
                    return;
                if (typeofPrim.primitive === 'undefined')
                    return;
                if (isUndeclaredIdentifierProbe(typeofPrim.operand, declaredIdentifiers))
                    return;
                const line = lineOf(sourceFile, node);
                if (typeofPrim.primitive === 'object') {
                    ...
                    const negated = op === ts.SyntaxKind.ExclamationEqualsEqualsToken ||
                        op === ts.SyntaxKind.ExclamationEqualsToken;
                    if (!negated && shouldSkipTypeofObjectStrictEqBinary(node, sourceFile))
                        return;
                    if (negated && shouldSkipTypeofObjectNotEqForRejectTriple(node, sourceFile))
                        return;
                ...
                if (typeofPrim.primitive === 'number') {
                    if (!negated && shouldSkipTypeofNumberEqForFiniteAndPair(node, sourceFile))
                        return;
                    if (!negated && shouldSkipTypeofNumberEqForNumberOrNullTernary(node, sourceFile))
                        return;
                    if (negated && shouldSkipTypeofNumberNeForFiniteOrPair(node, sourceFile))
                        return;
                }
                if (typeofPrim.primitive === 'string') {
                    if (!negated && shouldSkipTypeofStringEqForNonBlankAndPair(node, sourceFile))
                        return;
                    if (!negated &&
...
function typeofPrimitiveToGuard(primitive) {
    switch (primitive) {
        case 'string':
            return 'isString';
        case 'number':
...
function fixReplaceRange(relativePath, node, replacement, helpers) {
...
function tryParseTypeofEqualsPrimitive(node, primitive) {
    if (!ts.isBinaryExpression(node))
        return undefined;
    const op = node.operatorToken.kind;
...
function isUnderTypeofObjectMerge(node) {
...
function readTypeofComparedToNonUndefinedString(node) {
    const op = node.operatorToken.kind;
    if (op !== ts.SyntaxKind.EqualsEqualsEqualsToken &&
        op !== ts.SyntaxKind.ExclamationEqualsEqualsToken &&
        op !== ts.SyntaxKind.EqualsEqualsToken &&
...
function isUndeclaredIdentifierProbe(operand, declaredIdentifiers) {
    return ts.isIdentifier(operand) && !declaredIdentifiers.has(operand.text);
}
function collectRuntimeDeclaredIdentifiers(sourceFile) {
    const names = new Set();
...
function formatGuardOperand(expression, sourceFile) {
...
//# sourceMappingURL=forbidRawRuntimeTypeGuards.js.map